...
const ArchiveBlockingView: React.FC<Props> = ({ currentUser }) => {
  ...
  const fetchBlockingRecords = async () => {
    ...
    setLoading(true);
    try {
      const { data, error } = await supabase.from('archive_blocking_records').select('*').order('created_at', { ascending: false });
      if (error) throw error;
      setRecords(data as LandRecord[]);
      localStorage.setItem('offline_archive_blocking_records', JSON.stringify(data));
    } catch (error) {
