...
const ArchiveBlockingView: React.FC<Props> = ({ currentUser }) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (isConfigured) {
          ...
          localStorage.setItem('offline_archive_blocking_records', JSON.stringify(updatedOffline));
        }
      } catch (error) {
  ...
  const fetchBlockingRecords = async () => {
    ...
    try {
      ...
      localStorage.setItem('offline_archive_blocking_records', JSON.stringify(allRecords));
    } catch (error) {
  ...
  const handleSave = async (formData: any) => {
    try {
      ...
      else localStorage.setItem('offline_archive_blocking_records', JSON.stringify(records));
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setRecords(newRecs);
      if (!isConfigured) localStorage.setItem('offline_archive_blocking_records', JSON.stringify(newRecs));
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      setRecords([]);
      localStorage.setItem('offline_archive_blocking_records', JSON.stringify([]));
    ...
    <div className="h-full flex flex-col bg-gray-50 rounded-lg p-2 md:p-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div className="flex items-center gap-3">
            ...
            <div>
                 <h2 className="text-xl font-bold text-gray-800 tracking-tight">Quản lý ngăn chặn (Lưu trữ)</h2>
                 <p className="text-sm text-gray-500 font-medium">Quản lý các hồ sơ bị ngăn chặn, tranh chấp trong lưu trữ.</p>
            </div>
        </div>
