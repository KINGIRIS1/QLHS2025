...
export const saveLocalBackupPoint = async (
    ...
    try {
        ...
        // Async auto push to Google Drive if connected
        getGoogleDriveStatus().then(status => {
            if (status.connected) {
                const dateStr = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 16);
                uploadBackupToGoogleDrive(backupData, `Auto_Backup_QLHS_${dateStr}.json`)
                    .then(res => {
                        if (res.success) {
                            console.log("Auto-backed up to Google Drive:", res.message);
                        }
                    })
                    .catch(err => console.warn("Google Drive auto push error:", err));
            }
        });
...
// --- GOOGLE DRIVE INTEGRATION API CALLS ---
...
export const getGoogleDriveAuthUrl = async (): Promise<string | null> => {
    try {
        ...
        console.error("Error getting Google OAuth URL:", e);
        return null;
    }
};
...
export const uploadBackupToGoogleDrive = async (
    backupData?: BackupData, 
    fileName?: string
): Promise<{ success: boolean; message?: string; file?: GoogleDriveFile; error?: string }> => {
    try {
        const payload: any = {};
        if (backupData) {
            payload.backupContent = backupData;
        }
        if (fileName) {
            payload.fileName = fileName;
        }

        const res = await fetch('/api/backup/google-drive/upload', {
            method: 'POST',
        ...
        const data = await res.json();
        if (!res.ok) {
            return { success: false, error: data.error || 'Lỗi khi tải bản sao lưu lên Google Drive.' };
        }
...
export const fetchGoogleDriveBackupFiles = async (): Promise<GoogleDriveFile[]> => {
    try {
        const res = await fetch('/api/backup/google-drive/files');
        if (!res.ok) return [];
        const data = await res.json();
        return data.files || [];
    } catch (e) {
        console.error("Error fetching Google Drive backup files:", e);
        return [];
    }
};
