...
import { calculateNextRecordCode, getWardShortCode as getShortCodeUtil } from './codeGenerator';
...
export const calculateDeadlineHelper = (type: string, receivedDateStr: string, holidays: Holiday[]): string => {
    ...
    if (lowerType.includes('hiến đất') || lowerType.includes('hien dat')) {
        ...
        daysToAdd = 10; 
    } else if (lowerType.includes('trích đo chỉnh lý') || lowerType.includes('chuyển mục đích') || lowerType.includes('chuyen muc dich') || lowerType.includes('tdcmd')) {
        daysToAdd = 15; 
...
export function generateNextRecordCode(
    ...
    return calculateNextRecordCode(wardName, dateStr, allRecords as any, extraCodes, recordType);
}
